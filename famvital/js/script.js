...
window.addEventListener('load', function () {
  ...
  function setupMenuUnderline(matchMedia) {
    ...
    // слушаем только изменение брейкпоинта
    matchMedia.addEventListener('change', toggle);
  }
  ...
  const shopsHide = (shopsCount) => {
    ...
    if (shops.length > shopsCount) {
      shopsList.classList.add('_is-shops-init');
      shopsList.classList.remove('_is-shops-lock');

      shopsShowBtn.onclick = (e) => {
        e.preventDefault();

        shopsList.classList.toggle('_is-shops-init');
      };
    } else {
      shopsList.classList.remove('_is-shops-init');
  ...
  if (document.querySelector('.categories-slider')) {
    ...
    new swiper_bundle__WEBPACK_IMPORTED_MODULE_3__["default"]('.categories-slider', {
      ...
      resistanceRatio: 0,
      wrapperClass: 'categories-slider__items',
      slideClass: 'categories-slider__item',
      slideActiveClass: 'categories-slider__item_active',
      slidePrevClass: 'categories-slider__item_prev',
      slideNextClass: 'categories-slider__item_next',
      pagination: {
        clickable: true,
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!isTargetMenu) {
      ...
      if (isOpenedMenu) {
        document.querySelectorAll('.menu [data-spoller-close]').forEach((spollerClose) => {
          const spollersBlock = spollerClose.closest('[data-spollers]');

          if (spollersBlock.classList.contains('_spoller-init')) {
            spollerClose.classList.remove('_spoller-active');
            spollerClose.nextElementSibling.hidden = true;
            spollerClose.parentNode.open = false;
            spollersBlock.classList.remove('_spollers-active');
          }
        });
      }
