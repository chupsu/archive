...
function tabs() {
  ...
  tabs.forEach((tabsBlock) => {
    // Инициализируем только новые табы
    if (!tabsBlock.classList.contains('_tab-init')) {
      tabsBlock.classList.add('_tab-init');

      // Назначаем уникальный индекс
      const existingIndexes = Array.from(document.querySelectorAll('[data-tabs-index]')).map((el) =>
        Number(el.dataset.tabsIndex),
      );
      const newIndex = existingIndexes.length ? Math.max(...existingIndexes) + 1 : 0;
      tabsBlock.setAttribute('data-tabs-index', newIndex);
  ...
  // Инициализация медиа-запросов для спойлеров
  ...
  function setTitlePosition(tabsMediaArray, matchMedia) {
    tabsMediaArray.forEach((tabsMediaItem) => {
      ...
      const tabsContentItems = Array.from(tabsMediaItem.querySelectorAll('[data-tabs-item]')).filter(
        (item) => item.closest('[data-tabs]') === tabsMediaItem,
      );
  ...
  function initTabs(tabsBlock) {
    let tabsTitles = Array.from(tabsBlock.querySelectorAll('[data-tabs-titles] > *')).filter(
      (item) => item.closest('[data-tabs]') === tabsBlock,
    );
    let tabsContent = Array.from(tabsBlock.querySelectorAll('[data-tabs-body] > *')).filter(
      (item) => item.closest('[data-tabs]') === tabsBlock,
    );

    const tabsBlockIndex = Number(tabsBlock.dataset.tabsIndex);
    const tabsActiveHashBlock = tabsActiveHash[0] == tabsBlockIndex;

    // Сброс активного класса, если есть хеш
    if (tabsActiveHashBlock) {
      const activeTitle = tabsBlock.querySelector('[data-tabs-titles] > ._tab-active');
  ...
  function setTabsStatus(tabsBlock) {
    let tabsTitles = Array.from(tabsBlock.querySelectorAll('[data-tabs-title]')).filter(
      (item) => item.closest('[data-tabs]') === tabsBlock,
    );
    let tabsContent = Array.from(tabsBlock.querySelectorAll('[data-tabs-item]')).filter(
      (item) => item.closest('[data-tabs]') === tabsBlock,
    );

    const isAnimate = tabsBlock.hasAttribute('data-tabs-animate')
      ? Number(tabsBlock.dataset.tabsAnimate) || 500
      : 0;
    const isHash = tabsBlock.hasAttribute('data-tabs-hash');

    tabsContent.forEach((tabsContentItem, index) => {
      if (tabsTitles[index].classList.contains('_tab-active')) {
        if (isAnimate) _slideDown(tabsContentItem, isAnimate);
        else tabsContentItem.hidden = false;

        if (isHash && !tabsContentItem.closest('.popup')) {
          setHash(`tab-${tabsBlock.dataset.tabsIndex}-${index}`);
        }
      } else {
        if (isAnimate) _slideUp(tabsContentItem, isAnimate);
  ...
  function setTabsAction(e) {
    ...
    if (tabTitle.classList.contains('_tab-active') || tabsBlock.querySelector('._slide')) return;

    const activeTitles = Array.from(tabsBlock.querySelectorAll('[data-tabs-title]._tab-active')).filter(
      (item) => item.closest('[data-tabs]') === tabsBlock,
    );
    activeTitles[0]?.classList.remove('_tab-active');
...
/* harmony import */ var _panzoom_panzoom__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(37);
...
window.initTabs = _module_functions_js__WEBPACK_IMPORTED_MODULE_0__.tabs;
...
window.addEventListener('load', function () {
  ...
  const matchMediaMi12 = window.matchMedia(`(min-width: ${$md12}px)`);
  ...
  matchMediaMi12.addEventListener('change', () => {
  ...
  document.querySelectorAll('.schema-tabs__body').forEach((body) => {
    const panElement = body.querySelector('.schema-tabs__panzoom');
    const image = panElement.querySelector('img');

    if (!image) return;

    const panzoom = (0,_panzoom_panzoom__WEBPACK_IMPORTED_MODULE_4__["default"])(image, {
      maxScale: 6,
      minScale: 1,
      contain: 'outside',
    });

    panElement.addEventListener('wheel', panzoom.zoomWithWheel);

    body.querySelector('.schema-tabs__zoom_in')?.addEventListener('click', () => {
      panzoom.zoomIn();
    });

    body.querySelector('.schema-tabs__zoom_out')?.addEventListener('click', () => {
      panzoom.zoomOut();
    });
  });

  if (document.querySelector('.places-slider')) {
    document.querySelector('.places-slider').classList.add('_is-slider-init');

    new swiper_bundle__WEBPACK_IMPORTED_MODULE_3__["default"]('.places-slider', {
      slidesPerView: 'auto',
      spaceBetween: 8 + 24 * ((document.documentElement.offsetWidth - 360) / (1920 - 360)),
      resistanceRatio: 0,
      wrapperClass: 'places-slider__items',
      slideClass: 'places-slider__item',
      slideActiveClass: 'places-slider__item_active',
      slidePrevClass: 'places-slider__item_prev',
      slideNextClass: 'places-slider__item_next',
      navigation: {
        prevEl: '.places-slider__btn_prev',
        nextEl: '.places-slider__btn_next',
        disabledClass: 'places-slider__btn_disabled',
        lockClass: '_is-lock',
      },
      on: {
        init: (swiper) => {
        ...
        resize: (swiper) => {
          swiper.params.spaceBetween =
            8 + 24 * ((document.documentElement.offsetWidth - 360) / (1920 - 360));
          swiper.el.classList.toggle('_is-slider-lock', swiper.isLocked);
        },
      },
    });
  ...
  if (document.querySelector('.skills-slider')) {
    document.querySelector('.skills-slider').classList.add('_is-slider-init');

    new swiper_bundle__WEBPACK_IMPORTED_MODULE_3__["default"]('.skills-slider', {
      slidesPerView: 'auto',
      spaceBetween: 8 + 24 * ((document.documentElement.offsetWidth - 360) / (1920 - 360)),
      resistanceRatio: 0,
      wrapperClass: 'skills-slider__items',
      slideClass: 'skills-slider__item',
      slideActiveClass: 'skills-slider__item_active',
      slidePrevClass: 'skills-slider__item_prev',
      slideNextClass: 'skills-slider__item_next',
      navigation: {
        prevEl: '.skills-slider__btn_prev',
        nextEl: '.skills-slider__btn_next',
        disabledClass: 'skills-slider__btn_disabled',
        lockClass: '_is-lock',
      },
      on: {
        init: (swiper) => {
  ...
  if (document.querySelector('.partners-slider')) {
    const partnersSliderBody = document.querySelector('.partners-slider');
    let partnersSlider;

    function responsiveInit() {
      if (window.innerWidth < $md2 && partnersSliderBody.dataset.slider == 'false') {
        partnersSliderBody.classList.add('_is-slider-init');

        partnersSlider = new swiper_bundle__WEBPACK_IMPORTED_MODULE_3__["default"](partnersSliderBody, {
          slidesPerView: 'auto',
          spaceBetween: 8 + 24 * ((document.documentElement.offsetWidth - 360) / (1920 - 360)),
          resistanceRatio: 0,
        ...
        partnersSliderBody.dataset.slider = 'true';
      }
      if (window.innerWidth >= $md2) {
        partnersSliderBody.dataset.slider = 'false';
        if (partnersSlider) {
          partnersSliderBody.classList.remove('_is-slider-init');
  ...
  document.addEventListener('click', (e) => {
    ...
    dataAction(targetElement, 'schema-tabs');
    if (targetElement.closest('.schema-tabs__title') && window.innerWidth < $md4) {
      dataAction(document.querySelector('[data-schema-tabs]'), 'schema-tabs');
    }
