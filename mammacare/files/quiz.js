(function () {
  ...
  quizzes.forEach((form) => {
    const steps = form.querySelectorAll('.quiz__step');
    const pagination = form.querySelector('.quiz__pagination');
    const btnBack = form.querySelector('.quiz__btn_back');
    const btnNext = form.querySelector('.quiz__btn_next');
    const btnSubmit = form.querySelector('.quiz__btn_submit');
    ...
    function updateUI() {
      ...
      // Обновляем пагинацию: закрашиваем точки от первой до текущей включительно.
      if (pagination) {
        const dots = pagination.querySelectorAll('span');
        dots.forEach((dot, idx) => {
          dot.classList.toggle('_is-active', idx <= currentStepIndex);
        });
      }
