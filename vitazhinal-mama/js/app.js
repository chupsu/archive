(() => {
	...
	const tableRows = document.querySelectorAll(".table-intro__row");
	const tableNames = document.querySelectorAll(".table-intro__name");
	const arrowLeft = document.querySelector(".intro__arrow_left");
	const arrowRight = document.querySelector(".intro__arrow_right");
	if (tableRows.length && arrowLeft && arrowRight && tableNames.length) {
		let current = 0;
		arrowRight.addEventListener("click", (e => {
			e.preventDefault();
			if (current < tableNames.length - 1) {
				current++;
				if (current == tableNames.length - 1) arrowRight.classList.add("_disabled");
				tableRows.forEach((tableRow => {
					const tableItems = tableRow.querySelectorAll(".table-intro__item");
					if (tableItems.length) {
						tableItems.forEach((tableItem => {
							tableItem.classList.remove("_active");
						}));
						tableItems[current].classList.add("_active");
					}
					tableNames.forEach((tableName => {
						tableName.classList.remove("_active");
					}));
					tableNames[current].classList.add("_active");
					arrowLeft.classList.remove("_disabled");
				}));
			}
		}));
		arrowLeft.addEventListener("click", (e => {
			...
			if (current > 0) {
				current--;
				if (current == 0) arrowLeft.classList.add("_disabled");
				tableRows.forEach((tableRow => {
					const tableItems = tableRow.querySelectorAll(".table-intro__item");
					if (tableItems.length) {
						tableItems.forEach((tableItem => {
					...
					tableNames[current].classList.add("_active");
					arrowRight.classList.remove("_disabled");
				}));
			}
		}));
	}
		...
		if (slideParents.length) {
			slideParents.forEach(slideParent => {
				if (slideParent.classList.contains('swiper-slide-active') || slideParent.dataset.slideParent == 'static') {
					...
					if (slidePanel && !e.target.closest('[data-slide-more]') && !e.target.closest('.click-btn') && !e.target.closest('.panel-intro__main') && !e.target.closest('.intro__back') && !e.target.closest('.item') && !e.target.closest('.btn-gg')) {
