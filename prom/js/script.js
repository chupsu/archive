...
(() => {
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _module_select_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);



window.addEventListener("load", function () {
  if (document.querySelector("[data-textarea-limit]")) {
    const limitTextareas = document.querySelectorAll("[data-textarea-limit]");

    limitTextareas.forEach((limitTextarea) => {
      const textarea = limitTextarea.querySelector("textarea");
      const limitTextareaIndicator = limitTextarea.querySelector("[data-textarea-limit-value]");
      const limitTextareaValue = parseInt(limitTextareaIndicator.dataset.textareaLimitValue);

      const controlTextareaLimit = (e) => {
        let textareaLength = e.target.value.length;

        if (textareaLength > limitTextareaValue) {
          e.target.value = e.target.value.substr(0, limitTextareaValue);
          textareaLength = e.target.value.length;
        }

        limitTextareaIndicator.innerHTML = `<span>${limitTextareaValue - textareaLength}</span>`;
      };

      if (limitTextareaValue) {
        limitTextareaIndicator.style.display = "block";
        limitTextareaIndicator.innerHTML = `<span>${limitTextareaValue}</span>`;

        textarea.addEventListener("input", controlTextareaLimit);
      }
    });
  }
});
